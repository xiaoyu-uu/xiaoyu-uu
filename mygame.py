...
start_game = False
game_mode = None
game_time = 30  # Set timer for 30 seconds
...
def gameMapInit(mode):
    global matrix, start_ticks, last_refresh_time
    row_count = 4 if mode == 'easy' else 7  # 4 rows for easy, 7 for hard
    for i in range(row_count):
        for j in range(7):
            matrix[i][j] = random.randint(1, 7)  # Fill with valid cards
    start_ticks = pygame.time.get_ticks()  # Get current ticks for timer
    last_refresh_time = start_ticks  # Initialize last refresh time
...
def reset_game():
    global start_game, matrix, game_mode, score
    start_game = False
    game_mode = None
...
while True:
    ...
    if not start_game:
        ...
        # Refresh cards every 5 seconds without resetting the score
        if pygame.time.get_ticks() - last_refresh_time > 5000:
        ...
        if (pygame.time.get_ticks() - start_ticks) // 1000 >= game_time:
    ...
    if not start_game and matrix[10].count(0) != 7:
        drawGameOver()
